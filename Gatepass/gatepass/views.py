...
def create_gatepass(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Create notification for appropriate warden
            appropriate_warden = gatepass.get_appropriate_warden()
            if appropriate_warden:
                Notification.objects.create(
                    user=appropriate_warden,
                    gatepass=gatepass,
                    notification_type='gatepass_request',
                    message=f"New gatepass request from {student.student_name}"
                )
            else:
                # If no specific warden found, notify all wardens
                all_wardens = User.objects.filter(role='warden', is_approved=True)
                for warden in all_wardens:
                    Notification.objects.create(
                        user=warden,
                        gatepass=gatepass,
                        notification_type='gatepass_request',
...
def warden_dashboard(request):
    ...
    # Apply gender filter if warden has gender set (but show all if no gender)
