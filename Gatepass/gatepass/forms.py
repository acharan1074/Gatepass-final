...
class SecurityReturnForm(forms.ModelForm):
    ...
    class Meta:
        model = GatePass
        fields = ['actual_return_date', 'actual_return_time', 'return_notes']
        widgets = {
            'actual_return_date': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}),
            'actual_return_time': forms.TimeInput(attrs={'class': 'form-control', 'type': 'time'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.actual_return_time:
            from django.utils import timezone
            self.fields['actual_return_time'].initial = timezone.now().time()
